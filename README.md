...
- 下单接口：`order_shares()`、`order_target_percent()`、`order_percent()`
- 回测结果可视化与指标统计
...
| 方法 | 说明 |
|------|------|
| `order_shares(sec, amount)` | 买入/卖出股数 |
| `order_percent(sec, percent)` | 按资金比例下单 |
| `order_target_percent(sec, percent)` | 调仓到目标比例 |
